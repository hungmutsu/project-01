...
{
	if (Auth::guest()) return Redirect::to('login')->with(array('note' => 'You must be logged in first before performing this action.', 'note_type' => 'error'));
});
...
{   
    if(!Config::get('app.debug')){
        include('content/themes/' . Config::get('site.theme') . '/error.php'); die();
    }
});
