...
class UserController extends BaseController{
	...
	public function login(){
	    ...
	    if ( Auth::attempt($email_login) || Auth::attempt($username_login) ){
	    	...
	    	if(Auth::user()->active){
		    	...
		    	Auth::logout();
		    	return Redirect::to('login')->with(array('note' => 'This account is no longer active', 'note_type' => 'error'));
		    }
	    	
	    } else {
	        // auth failure! redirect to login with errors
	        return Redirect::to('login')->with(array('note' => Lang::get('lang.signin_error'), 'note_type' => 'error'));
	    }
	...
	public function password_reset_post()
	{
	  ...
	  return Password::reset($credentials, function($user, $password)
	  {
	    ...
	    return Redirect::to('login')->with(array('note' => Lang::get('lang.password_reset'), 'note_type' => 'success'));
	  });
	}
