...
class InstallController extends BaseController {
	...
	public function upgrade(){
		...
		if(file_exists('./upgrade_script/index.php')){
			unlink('./upgrade_script/index.php');
			unlink('./upgrade_script/upgrade.php');
			rmdir('./upgrade_script/');
		}
		...
		$upgraded = $this->upgrade111();
		$upgraded = $this->upgrade115();
		$upgraded = $this->upgrade150();
	...
	public function upgrade108(){
		...
		if(!Schema::hasColumn('settings', 'enable_watermark')){
			...
			Schema::table('settings', function($table)
			{
				...
				$table->string('watermark_image')->default('watermark.png');
	...
	public function upgrade115(){
		...
		{
		    Schema::create('plugin_data', function($table) {
				$table->increments('id');
	...
	public function upgrade150(){
		...
		if( !Schema::hasColumn('settings', 'system_email')){

			Schema::table('settings', function($table)
			{
				$table->string('system_email')->default('email@email.com');
			});
