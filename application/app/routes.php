...
Route::group(array('before' => 'admin'), function(){
	...
	Route::get('perform_upgrade', function(){
		...
		$files = scandir("application/upgrade_script/");
		// Identify directories
		$source = "application/upgrade_script/";
		$destination = "upgrade_script/";

		if(!file_exists($destination)){
			if (!mkdir($destination, 0777, true)) {
			    die('Failed to create upgrade folder');
			}
		}
		...
		foreach ($files as $file) {
		  ...
		  if (copy($source.$file, $destination.$file)) {
		  	// remove the first line from the new destination to allow it to be accessed
		  	read_and_delete_first_line($destination.$file);
		    $delete[] = $source.$file;
		  }
		}
		return Redirect::to('upgrade_script/index.php');
	});
